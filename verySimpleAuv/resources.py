...
def trainAgent(agent, nTrainingSteps, log_file, log_interval=100000000, progress_bar=True):
	...
	convergenceData = pandas.read_csv(log_file+"monitor.csv", skiprows=1)
	
	print(f"Final reward {convergenceData['r'].iloc[-1]}")

	return convergenceData, trainingTime

def saveHyperparameteres(agentName, agent_kwargs, policy_kwargs, env_kwargs, nTrainingSteps, trainingTimes, nProc, savefile):
	with open(savefile, "w") as outf:
		try:
			trainingTimes[0]
		except TypeError:
